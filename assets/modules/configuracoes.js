function pageConfiguracoes() {
  ...
  const iaSection = userRole !== 'funcionario' ? `
    <div class="cfg-card page-enter">
      <div class="cfg-card-header">
        <span class="cfg-card-icon">🤖</span>
        <div>
          <h3 class="cfg-card-title">Inteligência Artificial</h3>
          <p class="cfg-card-sub">Processamento seguro no servidor — sem configuração necessária</p>
        </div>
      </div>
      <div class="cfg-ia-status">
        <span class="tag tag-success">✅ IA disponível</span>
        <span style="color:var(--text-muted); font-size:13px;">Processada com segurança no servidor Agro Pro.</span>
      </div>
    </div>
  ` : '';
  ...
  // API Key removida do front-end — limpeza de segurança
